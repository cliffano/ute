var _ = require('underscore'),
  assetManager = require('connect-assetmanager'),
  assetHandlers = require('connect-assetmanager-handlers'),
  dateformat = require('dateformat'),
  ejs = require('ejs'),
  everyauth = require('everyauth'),
  express = require('express'),
  fs = require('fs'),
  runId = dateformat(new Date(), 'yyyymmddHHMMssLl');

function _NotFound(message) {
  Error.call(message);
}

// configure template engine, static resources, error handler
function config(app, opts) {
  
  // TODO: recursive asset file handling
  function isAsset(file) {
    return file.match(/^.+\..+$/);
  }

  app.configure(function () {

    // use ejs as default template engine
    app.register('.html', ejs);
    app.register('.json', ejs);
    app.register('.xml', ejs);

    // merged javascript and css should take precedence over individual static resources
    app.use(assetManager({
      'js': {
        'route': new RegExp('\\/scripts\\/' + opts.name + '\\.js'),
        'path': './public/scripts/',
        'dataType': 'javascript',
        'files': _.select(fs.readdirSync('./public/scripts/'), isAsset)
      },
      'css': {
        'route': new RegExp('\\/styles\\/' + opts.name + '\\.css'),
        'path': './public/styles/',
        'dataType': 'css',
        'files': _.select(fs.readdirSync('./public/styles/'), isAsset),
        'preManipulate': {
          'MSIE': [
            assetHandlers.yuiCssOptimize
          ],
          '^': [
            assetHandlers.yuiCssOptimize
          ]
        }
      }
    }));

    // expose all static resources
    // use ['static'] instead of .static to satisfy jshint
    // (ok, ok, I know not everyone is a fan of JavaScript linters)
    app.use('/', express['static']('./public/'));

    app.use(express.methodOverride());
    app.use(express.bodyParser());
    app.use(express.cookieParser());
  });

  // error 404 and 500
  app.error(function (err, req, res, next) {
    console.error('Error: ' + err.message);
    function render(status) {
      res.render(status + '.html', {
        locals: { err: err, env: process.env, runId: runId },
        status: status
      });
    }
    if (err instanceof _NotFound) {
      render(404);
    } else {
      render(500);
    }
  });
  app.use(express.errorHandler());
}

// _must_ be called after config and before route
function auth(app, opts) {

  app.use(express.session({ secret: opts.auth.sessionSecret }));
  
  var nextUserId = 0,
    usersById = {},
    usersByTwitterId = {};

  // shared cross everyauth sources
  function addUser (source, sourceUser) {
    var user = usersById[++nextUserId] = { id: nextUserId };
    user[source] = sourceUser;
    return user;
  }

  // enable user retrieval in handlers and view templates
  everyauth.everymodule
    .findUserById(function (id, cb) {
      cb(null, usersById[id]);
    });

  if (opts.auth.twitter) {
    everyauth.twitter
      .myHostname(opts.auth.twitter.hostName)
      .consumerKey(opts.auth.twitter.consumerKey)
      .consumerSecret(opts.auth.twitter.consumerSecret)
      .findOrCreateUser( function (session, accessToken, accessSecret, twitterUser) {
        return usersByTwitterId[twitterUser.id] ||
          (usersByTwitterId[twitterUser.id] = addUser('twitter', twitterUser));
      })
      .redirectPath(opts.auth.twitter.redirectPath);
  }

  app.use(everyauth.middleware());
  everyauth.helpExpress(app);
}

// set application routes and error routes
// opts.routes = [
//   method: 'get',
//   path: '/about',
//   cb: function () { ... }
// ];
function route(app, opts) {
  opts.routes.forEach(function (route) {
    app[route.method](route.path, function (req, res, next) {
      route.cb(req, res, next, { env: process.env, runId: runId, title: route.title || '' });
    });
  });

  // error routes should be set last as a catch all
  app.get('/500', function (req, res, next) {
    throw new Error('Unexpected error');
  });
  app.get('/*', function (req, res, next) {
    throw new _NotFound();
  });
}

exports.config = config;
exports.auth = auth;
exports.route = route;